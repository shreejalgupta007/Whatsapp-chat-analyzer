...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
       ...
       st.pyplot(fig)
        ...
        # finding the busiest users (group chat)
       ...
       with col1:
           fig,ax=plt.subplots()
           labels=list(emoji_df[0])
           p1=ax.bar(np.arange(len(labels)), emoji_df[1])
           
           #adding labels
           for rect1, label in zip(p1, labels):
               height=rect1.get_height()
               plt.annotate(label, (rect1.get_x()+rect1.get_width()/2, height+.05), fontsize=10)
           st.pyplot(fig)
       with col2:
